    ...
    while True:
        ...
        if(len(siguiente)>=1):
            ...
            resultados_pagina = [
                ...
                total,
                total,
                f'Existen {cont_javasc} ({int((cont_javasc / total) * 100)}%) elementos de JavaScript y {cont_type} ({int((cont_type / total) * 100)}%) elementos de TypeScript,lenguajes diferentes {totalLenguajes}  ({int((cont_dif / totalLenguajes) * 100)}%) ',
                total
            ]
